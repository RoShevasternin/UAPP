package com.rbxgolden.fungamems.game.actors.panel.wheel
...
class AWheel(override val screen: AdvancedScreen) : AdvancedGroup() {
    ...
    private val aWheelBackImg  = Image(gdxGame.assetsAll.WHEEL_BACK)
    private val aWheelFrontImg = Image(gdxGame.assetsAll.WHEEL_FRONT)
...
//    var blockResult: (Result) -> Unit = {}
    ...
    override fun addActorsOnGroup() {
        ...
        addActor(aWheelBackImg)
        aWheelBackImg.setBounds(-12f, -12f, 416f, 416f)

        addAndFillActor(aWheelFrontImg)
        aWheelBackImg.setOrigin(Align.center)
    ...
    fun spin(blockResult: (winItem: Result) -> Unit) {
        ...
        aWheelBackImg.addAction(
            Actions.sequence(
                Actions.rotateBy(randomRotation, (2..4).random().toFloat(), Interpolation.exp5),
                Actions.run {
                    val degree = (aWheelBackImg.rotation.roundToInt().absoluteValue) % 360f
