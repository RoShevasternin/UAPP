...
class Onboarding_1_Screen: AdvancedScreen() {

    // ------------------------------------------------------------------------
    // Actors
    // ------------------------------------------------------------------------

    private val aContentImg by lazy { Image(gdxGame.assetsAll.listOnb[0]) }
    ...
    // ------------------------------------------------------------------------
    override fun show() {
        setBackBackground(gdxGame.assetsAll.listOnboarding[0])
    ...
    private fun AConstraintLayout.addNextBtn() {
        ...
        aNextBtn.setOnClickListener {
            animHideScreen {
                gdxGame.navigationManager.navigate(Onboarding_2_Screen::class.java.name, Onboarding_1_Screen::class.java.name)
            }
        }
        ...
        coroutine?.launch {
            AdSizeManager.bannerFlow.collect {
                runGDX {
                    ...
                    log("Onboarding_1_Screen: banner = ${screen.safeBannerUI}")
                }
            }
        }
    }
