...
//    var blockResult: (Result) -> Unit = {}
    ...
    override fun addActorsOnGroup() {
        disable()
        addActor(aWheelImg)
        aWheelImg.setSize(412f, 412f)
        aWheelImg.setOrigin(Align.center)

        val aTargetImg = Image(gdxGame.assetsAll.WHEEL_FRONT)
        addAndFillActor(aTargetImg)
    }
    ...
    fun spin(blockResult: (winItem: Result) -> Unit) {
        ...
        aWheelImg.addAction(
            Actions.sequence(
                Actions.rotateBy(randomRotation, (2..4).random().toFloat(), Interpolation.exp10Out),
                Actions.run {
