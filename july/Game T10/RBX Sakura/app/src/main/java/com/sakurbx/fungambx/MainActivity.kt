...
class MainActivity : AppCompatActivity(), AndroidFragmentApplication.Callbacks {
    ...
    fun openPrivacyPolicy() {
        runOnUiThread {
            val url = "https://doc-hosting.flycricket.io/rbx-sakura-privacy-policy/2b788d6c-bbf8-4619-a57f-3f6b3bc779ea/privacy"
            try {
                startActivity(
                    Intent(Intent.ACTION_VIEW).apply {
                        data  = url.toUri()
    ...
    private fun fetchRemoteConfig(onComplete: (success: Boolean) -> Unit) {
        ...
        remoteConfig.fetchAndActivate().addOnCompleteListener(this) { task ->
            if (task.isSuccessful) {
                runCatching {
                    val json = remoteConfig.getString("config")   // ← ім'я параметра
