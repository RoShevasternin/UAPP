...
class APanelSelectRank(override val screen: AdvancedScreen): AConstraintLayout(screen) {
    ...
    private val aContent = Image(gdxGame.assetsAll.listSelectContent[1])
    ...
    private val listItemBox = List(19) { Actor() }
    ...
    private fun AAutoLayout.addListBox() {
        listItemBox.forEachIndexed { index, box ->
            box.setSize(109f, 114f)
            add(box)

            box.setOnTouchListener {
                screen.animHideScreen {
                    gdxGame.navigationManager.navigate(SelectCategoryScreen::class.java.name, screen::class.java.name)
                }
            }
