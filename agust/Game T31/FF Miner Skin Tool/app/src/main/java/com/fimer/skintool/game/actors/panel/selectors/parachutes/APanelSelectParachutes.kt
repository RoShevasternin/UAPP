...
class APanelSelectParachutes(override val screen: AdvancedScreen): AConstraintLayout(screen) {
    ...
    private val aContent = Image(gdxGame.assetsAll.PARASHUTES)
        ...
        listItemBox.forEachIndexed { index, box ->
            ...
            box.setOnTouchListener {
                screen.animHideScreen {
                    GLOBAL_PARACHUTES_INDEX = index
                    gdxGame.navigationManager.navigate(ParachutesScreen::class.java.name, screen::class.java.name)
                }
            }
