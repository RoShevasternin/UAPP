...
class APanelSelectTips(override val screen: AdvancedScreen): AConstraintLayout(screen) {
    ...
    private val aContent = Image(gdxGame.assetsAll.TIPS)
    ...
    private val listItemBox = List(5) { Actor() }
    ...
    private fun AAutoLayout.addListBox() {
        listItemBox.forEachIndexed { index, box ->
            box.setSize(344f, 52f)
            add(box)

            box.setOnTouchListener {
                GLOBAL_TIPS_INDEX = index
                screen.animHideScreen {
                    gdxGame.navigationManager.navigate(TipsScreen::class.java.name, screen::class.java.name)
                }
            }
