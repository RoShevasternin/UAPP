...
class APanelSelectId(override val screen: AdvancedScreen): AConstraintLayout(screen) {
    ...
    private val aContent = Image(gdxGame.assetsAll.listSelectContent[0])
    ...
    private fun AAutoLayout.addListBox() {
        listItemBox.forEachIndexed { index, box ->
            box.setSize(344f, 64f)
            add(box)

            box.setOnTouchListener {
                screen.animHideScreen {
                    gdxGame.navigationManager.navigate(SelectRankScreen::class.java.name, screen::class.java.name)
                }
            }
