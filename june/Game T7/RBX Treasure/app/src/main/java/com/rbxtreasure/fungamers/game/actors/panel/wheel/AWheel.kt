package com.rbxtreasure.fungamers.game.actors.panel.wheel
...
class AWheel(override val screen: AdvancedScreen) : AdvancedGroup() {
    ...
    private val aBackImg  = Image(gdxGame.assetsAll.BACK)
    private val aWheelImg = Image(gdxGame.assetsAll.WHEEL)
    private val aFrontImg = Image(gdxGame.assetsAll.FRONT)
...
//    var blockResult: (Result) -> Unit = {}
    ...
    override fun addActorsOnGroup() {
        ...
        addAndFillActor(aBackImg)

        aWheelImg.setSize(422f, 422f)
        addActorAligned(aWheelImg, AlignH.CENTER, AlignV.CENTER)
        aWheelImg.setOrigin(Align.center)

        addActor(aFrontImg)
        aFrontImg.setBounds(232f, 343f, 93f, 124f)
    }
    ...
    fun spin(blockResult: (winItem: Result) -> Unit) {
        ...
        aWheelImg.addAction(
            Actions.sequence(
                Actions.rotateBy(randomRotation, (2..4).random().toFloat(), Interpolation.fade),
                Actions.run {
