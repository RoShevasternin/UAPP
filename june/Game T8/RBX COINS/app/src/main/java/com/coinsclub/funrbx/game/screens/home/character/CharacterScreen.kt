...
import com.coinsclub.funrbx.game.actors.panel.character.APanelCharacter
...
import com.coinsclub.funrbx.game.utils.global.GLOBAL_LIST_CHARACTER_DESCRIPTIONS
...
class CharacterScreen: AdvancedScreen() {
    ...
    private fun AConstraintLayout.addPanelCharacter() {
        aPanelCharacter.setSize(346f, 188f)
        add(aPanelCharacter) { centerX(); topToBottom(aPanelTop, 16f) }
    }

    private fun AConstraintLayout.addDescLbl() {
        aDescLbl.setSize(344f, 180f)
        add(aDescLbl) { centerX(); topToBottom(aPanelCharacter, 50f) }
        aDescLbl.wrap = true
